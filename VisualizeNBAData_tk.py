...
class VisualizeNBAData:
    ...
    def build_window(self):
        ...
        記得按下「選定」確認選項
        --主球隊為必選，比較球隊可以不選擇--
        --歷年季賽、歷年季後賽將以主球隊的數據呈現--""", font=WIDGET_FONT).pack()
        Label(window, text="", height=1).pack()
        frame.pack()

        ##------first team------##
        Label(frame, text="主球隊：", font=WIDGET_FONT).grid(row=0, column=0)
        ...
        ##------second team------##
        Label(frame, text="比較球隊：", font=WIDGET_FONT).grid(row=2, column=0)
        second_team_select = ttk.Combobox(frame, font=WIDGET_FONT, values=["(不選擇)", *self._teamlist], width=12, state='readonly')
        ...
        ##------plot method------##
        Label(frame, text="查看類別：", font=WIDGET_FONT, width=10).grid(row=4, column=0)
        plot_method_select = ttk.Combobox(frame, font=WIDGET_FONT, values=["歷年季賽", "歷年季後賽", "球隊季賽比較"],
        ...
        ##-----------------------##
        plot_button = Button(window, text="作圖!", font=WIDGET_FONT, relief=RAISED, bd=3 , command=plot_fig)
        Button(window, text="離開", font=BUTTON_FONT, relief=RAISED, bd=3, command=_quit).place(x=620, y=540)
